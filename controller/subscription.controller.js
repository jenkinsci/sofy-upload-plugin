...
const btTokenGeneration = async (userGuid) => {
  ...
  if (userDataFromUserBrainTreeDetails.recordset.length == 0) {
    ...
    if (userDataFromUsers.recordset.length != 0) {
      ...
      const resultFromBrainTreeUserCreationMethod = await userCreationOnBrainTree;
...
const createSubscription = async (paymentMethodNonce, planId, user_guid) => {
  ...
  if (userDataFromBrainTreeTable.recordset.length == 0) {
    ...
    if (subscriptionCreationObject.success) {
      ...
      const {
        billingPeriodEndDate, id, billingPeriodStartDate, firstBillingDate, billingDayOfMonth, planId,
      } = subscriptionCreationObject.subscription;
      const userEmail = subscriptionCreationObject.subscription.transactions[0].customer.email;
      const spCreateSubscriptionResult = await db.request()
    ...
    const paidThroughDateFromBTData = moment(userDataFromBrainTreeTable.recordset[0].PaidThroughDate);
    const userSubscriptionsID = userDataFromBrainTreeTable.recordset[0].SubscriptionBTID;
    ...
    if (subscriptionCancelStatus.errors == null || subscriptionCancelStatus.message == 'Subscription has already been canceled.') {
      ...
      if (twoPlans.recordset[1].BTreeCode == planId || ((paidThroughDateFromBTData.diff(currentdate, 'days')) < 1)) {
        ...
        const dayDiff = paidThroughDateFromBTData.diff(currentdate, 'days');
        const futureDate = currentdate.add(dayDiff + 2, 'days');
...
const cancelSubscription = async (userGuid) => {
  ...
  if (currentSubscriptionID.lenght != 0) {
    const subscriptionIDFromDB = currentSubscriptionID.recordset[0].subscriptionBTID;
...
const brainTreeWebHookHandler = async (bt_signature, bt_payload) => {
  ...
  const subscriptionBTID = webhookNotification.subscription.id;
  ...
  if (notificationType == 'subscription_charged_successfully') {
    ...
    const userEmail = webhookNotification.subscription.transactions[0].customer.email;
